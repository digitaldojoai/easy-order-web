...
function MenuItem({
  ...
  const linkClass = classNames({
    ...
    ["py-2 xl:py-3 font-medium text-sm text-slate-700 dark:text-slate-100 [&.active]:dark:text-blue-600 group-hover:text-blue-600"]:
      !dropdown,
    ["py-2 px-4 text-xs text-slate-600 dark:text-slate-200 [&.active]:dark:text-blue-600"]:
      dropdown,
  });
  ...
  const dropdownIconClass = classNames({
    ...
    ["-rotate-90 origin-center ms-auto"]:
      label == "3" && theme.direction === "ltr",
    ["rotate-90 origin-center ms-auto"]:
      label == "3" && theme.direction === "rtl",
  });
  ...
  return divider ? (
    <li className="my-2 border-t border-slate-200 dark:border-slate-800"></li>
  ) : heading ? (
      ...
      {sub ? (
        <>
          <a href={link} className={linkClass} onClick={(e) => menuToggle(e)}>
            <span>{text}</span>
            {tag && (
              <span className="ms-1 rounded-sm bg-slate-200 px-2 py-1 text-[10px] leading-3 text-slate-600 dark:bg-slate-800 dark:text-slate-200">
                {tag}
              </span>
            )}
            <svg
